...
cval* builtin_head(cenv* env, cval* args) {
  ...
  CASSERT(args, args->sexpr->cell[0]->sexpr->count != 0,
...
cval* builtin_tail(cenv* env, cval* args) {
  ...
  CASSERT(args, args->sexpr->cell[0]->sexpr->count > 1,
          "func(tail): Index out of bounds, "
          "expected sexpr with more than 1 element");
